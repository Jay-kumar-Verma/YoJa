# Enhanced Secure Docker Build Script for Backend Service
# This script builds and validates the ultra-secure Backend Docker image

param(
    [string]$ImageName = "yoja/backend-service:secure-v2",
...
Write-Log "Starting enhanced secure Backend Service Docker build process"
...
if (!(Test-Path "app")) {
    Write-Error-Log "Application directory 'app' not found"
    exit 1
}
...
try {
    ...
    Write-Log "Building secure Backend Docker image..."
    ...
    if ($Test) {
        ...
        $testResult = docker run --rm --user nonroot:nonroot $ImageName python -c "import sys; print('Python version:', sys.version_info); print('Basic test passed')" 2>$null
        if ($LASTEXITCODE -eq 0) {
...
" 2>$null
        ...
        Write-Log "Test 4: Application structure test"
        $appTest = docker run --rm --user nonroot:nonroot $ImageName python -c "
import os
if os.path.exists('/app') and os.path.exists('/app/app'):
    print('Application structure verified')
    exit(0)
else:
    print('Application structure validation failed')
    exit(1)
" 2>$null
        if ($LASTEXITCODE -eq 0) {
            Write-Success "Application structure test passed"
        } else {
            Write-Warning "Application structure test failed"
        }
    }
    ...
    if ($Scan) {
        ...
        $packageManagers = docker run --rm --user nonroot:nonroot $ImageName which apt-get apt yum apk 2>$null
        if ($packageManagers) {
            Write-Warning "Package managers detected in image"
        } else {
            Write-Success "No package managers found (secure)"
        }
    }
    ...
    docker tag $ImageName "yoja/backend-service:latest-secure" >$null 2>&1
    docker tag $ImageName "yoja/backend-service:$(Get-Date -Format 'yyyyMMdd')" >$null 2>&1
    ...
    if ($Push) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            docker push "yoja/backend-service:latest-secure" >$null 2>&1
            docker push "yoja/backend-service:$(Get-Date -Format 'yyyyMMdd')" >$null 2>&1
        } else {
    ...
    $buildReport = @{
        ...
        tests_passed = $Test
        pushed = $Push
        tags = @($ImageName, "yoja/backend-service:latest-secure", "yoja/backend-service:$(Get-Date -Format 'yyyyMMdd')")
    }
    
    $buildReport | ConvertTo-Json -Depth 3 | Out-File $reportPath -Encoding UTF8
    Write-Success "Build report saved to: $reportPath"
    
    Write-Success "Enhanced secure Backend Service build completed successfully!"
    Write-Log ""
    Write-Log "Run Instructions:"
    Write-Log "Development: docker run -d -p 8000:8000 --name backend-service-dev $ImageName"
    Write-Log ""
    Write-Log "Production (Maximum Security):"
    Write-Log "docker run -d -p 8000:8000 --name backend-service-secure \"
    ...
    Write-Log "  $ImageName"
    Write-Log ""
    Write-Log "Health Check: curl http://localhost:8000/health"
    Write-Log ""
    Write-Log "With Database:"
    Write-Log "docker-compose -f docker-compose.secure.yml up backend-service-prod db"
